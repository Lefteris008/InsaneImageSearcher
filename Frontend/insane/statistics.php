...
<html lang="en">
  <head>
    ...
    <script type="text/javascript">
        ...
        function drawHistogram(histogramId, imgEl) {
        window.addEventListener('load', function () {
        var histCanvas = document.getElementById(histogramId),
  ...
  drawHistogram('histogram', 'myImg');
  //Draw the second histogram
  drawHistogram('histogram2', 'myImg2');
      document.title = "IIS | Search Image Histogram"
   </script>

  <style type="text/css"><!--
#histogram, #myImg, #histogram2 {
...
#histogram.thumb, #histogram2.thumb {
  max-width: 400px;
  max-height: 400px;
}
...
#runtime { clear: both }
  ...
  <body>
      <?php if(isset($_GET['search_image'])) {?>
    <div id="title">Histogram of search image</div>
      <?php } else { ?>
    <div id="title">Histogram of image result</div>
    <div id="details3">Compare the histogram of the image result (left) and the query image (right)</div>
      <?php } ?>
    <div id="statistics-form-div">
   
    <?php if(isset($_GET['search_image'])) {
        $src = "search/search_image.jpg";
    } else {
        $src = $_GET['image'];
    }
?>
    <p><a href="<?php echo $src; ?>">
            <img class="thumb" id="myImg" src="<?php echo $src; ?>" width="250px"></a></p>

    <p><canvas class="thumb" id="histogram" width="800" height="680">Your 
    browser does not have support for Canvas.</canvas></p>
    
    <?php if(!isset($_GET['search_image'])) { ?>
    
    <img class="thumb" id="myImg2" src="search/search_image.jpg" style="display: none;">
    
    <p><canvas class="thumb" id="histogram2" width="800" height="680">Your 
    browser does not have support for Canvas.</canvas></p>
